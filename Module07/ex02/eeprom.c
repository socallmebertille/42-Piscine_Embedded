...
void EEPROM_hexdump(void)
{
    ...
    while (addr < 1024)
    {
        uart_print_hex((addr >> 8) & 0xFF); // octet haut de addr
        uart_print_hex(addr & 0xFF);        // octet bas de addr
        uart_printstr("  ");

        uint8_t line[16];
        for (uint8_t i = 0; i < 16; i++)   // get all octet at a addr by line
            line[i] = EEPROM_read(addr + i);

        for (uint8_t i = 0; i < 16; i++)   // hex vesion
        {
        ...
        for (uint8_t i = 0; i < 16; i++)   // ascii version
        {
            char c = line[i];
            if (c >= 32 && c <= 126)
                uart_tx(c);
